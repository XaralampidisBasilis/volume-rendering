...
block.coords = ivec3((block.exit_position + u_intensity_map.spacing * 0.5) * u_maxima_distance_map.inv_spacing);

// Sample the distance map and compute if block is occupied
vec2 block_data = texelFetch(u_textures.maxima_distance_map, block.coords, 0).rg;
block.cheby_distance = int(round(block_data.g * 255.0));
block.max_intensity = block_data.r;
...
block.min_position = (vec3(block.min_coords + 0) - MILLI_TOLERANCE) * u_maxima_distance_map.spacing - u_intensity_map.spacing * 0.5;
block.max_position = (vec3(block.max_coords + 1) + MILLI_TOLERANCE) * u_maxima_distance_map.spacing - u_intensity_map.spacing * 0.5;  
