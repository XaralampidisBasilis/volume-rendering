...
block.max_intensity = texelFetch(u_textures.maxima_distance_map, block.coords, 0).r;

// Compute block bounding box position in model space  
block.min_position = vec3(block.coords + 0) * u_maxima_distance_map.spacing - u_intensity_map.spacing * 0.5;
block.max_position = vec3(block.coords + 1) * u_maxima_distance_map.spacing - u_intensity_map.spacing * 0.5;  
