...
description: |
    ...
    If MATERIAL_ANMIPTROPY is defined, it will also take into account the anisotropy direction.
...
vec3 reflection(const in vec3 _V, const in vec3 _N, const in float _roughness) {
        // Reflect
#ifdef MATERIAL_ANMIPTROPY
    vec3  anisotropicT = MATERIAL_ANMIPTROPY_DIRECTION;
    vec3  anisotropicB = MATERIAL_ANMIPTROPY_DIRECTION;

    #ifdef MODEL_VERTEX_TANGENT
    anisotropicT = normalize(v_tangentToWorld * MATERIAL_ANMIPTROPY_DIRECTION);
    anisotropicB = normalize(cross(v_tangentToWorld[2], anisotropicT));
    #endif

    vec3  anisotropyDirection = MATERIAL_ANMIPTROPY >= 0.0 ? anisotropicB : anisotropicT;
    vec3  anisotropicTangent  = cross(anisotropyDirection, _V);
    vec3  anisotropicNormal   = cross(anisotropicTangent, anisotropyDirection);
    float bendFactor          = abs(MATERIAL_ANMIPTROPY) * saturate(5.0 * _roughness);
