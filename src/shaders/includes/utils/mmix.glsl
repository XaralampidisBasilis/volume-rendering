...
vec2  mmix(const in float a, const in float b, const in vec2  c) { return mix(vec2(a), vec2(b), c); }
vec3  mmix(const in float a, const in float b, const in vec3  c) { return mix(vec3(a), vec3(b), c); }
vec4  mmix(const in float a, const in float b, const in vec4  c) { return mix(vec4(a), vec4(b), c); }
