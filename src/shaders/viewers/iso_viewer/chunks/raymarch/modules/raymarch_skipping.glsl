...
vec3 occumap_dims = vec3(textureSize(u_sampler.occumap, block.lod));
block.size = u_occupancy.occumap_size / occumap_dims;

for(int counter = 0; counter < u_debug.number; counter++)
{
    block.texel = trace.position / u_occupancy.occumap_size;
    block.coords = ivec3(floor(block.texel * occumap_dims));
    // block.occupancy = textureLod(u_sampler.occumap, vec3(u_debug.texel), float(block.lod)).r;
    block.occupancy = texelFetch(u_sampler.occumap, ivec3(u_debug.texel * occumap_dims), block.lod).r;
    ...
    if (!block.occupied)
    {
       ...
       block.skip_depth = intersect_box_max(block.min_position, block.max_position, trace.position, ray.direction);
        ...
        // trace.spacing = max(block.skip_depth + ray.min_spacing * 0.5, 0.01);
