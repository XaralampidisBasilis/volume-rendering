...
vec2  inside(in float a, in float b, in vec2  v) { return 1.0 - max(step(v, vec2(a)), step(b, v)); }
vec3  inside(in float a, in float b, in vec3  v) { return 1.0 - max(step(v, vec3(a)), step(b, v)); }
vec4  inside(in float a, in float b, in vec4  v) { return 1.0 - max(step(v, vec4(a)), step(b, v)); }
