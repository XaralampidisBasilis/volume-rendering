...
next_stepping = mix(raymarch.max_step_scaling, next_stepping, next_stepping > 0.0);
next_stepping = mix(raymarch.max_step_scaling, next_stepping, num_roots > 0);

// choose the minimum valid solution and clamp the result between the allowable stepping range.
trace.step_scaling = clamp(next_stepping, raymarch.min_step_scaling, raymarch.max_step_scaling);
