...
next_stepping = mix(vec3(raymarch.max_step_scale), next_stepping, greaterThan(next_stepping, vec3(0.0)));
next_stepping = mix(vec3(raymarch.max_step_scale), next_stepping, bvec3(num_roots > 0));
...
trace.step_scaling = clamp(trace.step_scaling, raymarch.min_step_scale, raymarch.max_step_scale);
