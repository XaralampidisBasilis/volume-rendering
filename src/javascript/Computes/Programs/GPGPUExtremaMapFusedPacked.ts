...
class GPGPUToHalfFloat implements GPGPUProgram 
{
    ...
    constructor(inputShape: [number, number, number, number, number]) 
    {
        ...
        void main() 
        {
            ...
            if (outputCoords.w == 0)      setOutput(blockMinMax.r);
            else if (outputCoords.w == 1) setOutput(blockMinMax.g);
            else if (outputCoords.w == 2) setOutput(blockMinMax.g);
            else if (outputCoords.w == 3) setOutput(blockMinMax.g);
        }
    `
    }
}
...
{
    const program = new GPGPUExtremaMap(interpolationMap.shape, blockSize)
