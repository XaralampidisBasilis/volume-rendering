...
class Gestures extends THREE.EventDispatcher {
    ...
    constructor( renderer ) {
        ...
        this.parameters = [ 0, 1 ].map( (i) => ({
            ...
            pointerBuffer:  new Array( Gestures.BUFFER_LENGTH ).fill().map( () => new THREE.Vector2() ),
            pointerSmooth:  new Array( Gestures.BUFFER_LENGTH ).fill().map( () => new THREE.Vector2() ),
    ...
    async onConnected( event ) {
        ...
        await delay( Gestures.CONTROLLER_DELAY ); // need this to avoid some transient phenomenon, without it 
    ...
    async onDisconnected( event ) {
        ...
        await delay( Gestures.CONTROLLER_DELAY );
    ...
    detectTap() {  
        ...
        if ( this.detector.tap.current ) {
            ...
            if ( not( this.parameters[0].duration < Gestures.TAP_DURATION_MAX )) return;
            if ( not( this.parameters[0].distance < Gestures.TAP_DISTANCE_MAX )) return;  
    ...
    detectPolyTap() {  
        ...
        if ( ! this.detector.polytap.start ) {
            ...
            this.detector.polytap.timeoutID = setTimeout( () => this.endGesture('polytap'), Gestures.POLYTAP_DURATION );
        ...
        if ( this.detector.polytap.end ) {
            ...
            this.delayDetector( Gestures.DETECT_DELAY ); 
    ...
    detectSwipe() {
        ...
        if ( ! this.detector.swipe.start ) {
            ...
            if ( not( this.parameters[0].pathDistance > Gestures.SWIPE_DISTANCE_MIN )) return;
            if ( not( this.parameters[0].pathSpeed > Gestures.SWIPE_PATH_SPEED_MIN )) return;
    ...
    detectHold() {        
        ...
        if ( ! this.detector.hold.start ) {
            ...
            if ( not( this.detector.gesture === undefined )) return;
            if ( not( this.detector.numConnected === 1 ) ) return;
            if ( not( this.parameters[0].pathDistance < Gestures.HOLD_START_DISTANCE_MAX ) ) return;
            if ( not( this.parameters[0].duration > Gestures.HOLD_START_DURATION_MIN ) ) return;
    ...
    detectPan() {
        ...
        if ( ! this.detector.pan.start ) {
            ...
            if ( not( this.detector.gesture === undefined )) return;
            if ( not( this.detector.numConnected === 1 ) ) return;
            if ( not( this.parameters[0].pathDistance > Gestures.PAN_START_DISTANCE_MIN )) return;
            if ( not( this.parameters[0].pathSpeed < Gestures.PAN_PATH_SPEED_MAX )) return;
    ...
    detectPinch() {
        ...
        if ( ! this.detector.pinch.start ) {
            ...
            if ( not( Math.abs( this.parametersDual.distanceOffset ) > Gestures.PINCH_START_DIAMETER_OFFSET_MIN ) ) return;
            if ( not( Math.abs( this.parametersDual.angleOffset ) < Gestures.PINCH_START_ANGLE_OFFSET_MAX ) ) return;  
            if ( not( Math.abs( this.parametersDual.radialSpeed ) < Gestures.PINCH_START_RADIAL_SPEED_MAX )) return;   
    ...
    detectTwist() {
        ...
        if ( ! this.detector.twist.start ) {
            ...
            if ( not( this.detector.gesture === undefined )) return;
            if ( not( this.detector.numConnected === 2 ) ) return;
            if ( not( this.parametersDual.distance > Gestures.TWIST_START_DISTANCE_MIN ) ) return;
            if ( not( Math.abs( this.parametersDual.distanceOffset ) < Gestures.TWIST_START_DIAMETER_OFFSET_MAX ) ) return;
            if ( not( Math.abs( this.parametersDual.angleOffset ) > Gestures.TWIST_START_ANGLE_OFFSET_MIN ) ) return;
    ...
    detectExplode() {
        ...
        if ( ! this.detector.explode.start ) {
            ...
            if ( not( this.detector.gesture === undefined )) return;
            if ( not( this.detector.numConnected === 2 )) return;
            if ( not( this.parametersDual.distanceOffset > Gestures.EXPLODE_DIAMETER_OFFSET_MIN )) return;
            if ( not( Math.abs( this.parametersDual.radialSpeed ) > Gestures.EXPLODE_RADIAL_SPEED_MIN )) return;
    ...
    detectImplode() {
        ...
        if ( ! this.detector.implode.start ) {
            ...
            if ( not( this.detector.gesture === undefined )) return;
            if ( not( this.detector.numConnected === 2 )) return;
            if ( not( this.parametersDual.distanceOffset  < Gestures.IMPLODE_DIAMETER_OFFSET_MAX )) return;
            if ( not( Math.abs( this.parametersDual.radialSpeed ) > Gestures.IMPLODE_RADIAL_SPEED_MIN )) return;
    ...
    printParameters( i, digits ) {
        ...
        console.log(`parameters${i}: \n\n\tpointerBuffer[ 0 ] = ${formatVector( this.parameters[i].pointerBuffer[0], digits )} mm \n\n\tpointerBuffer[end] = ${formatVector( this.parameters[i].pointerBuffer[Gestures.BUFFER_LENGTH - 1], digits )} mm`);
        console.log(`parameters${i}: \n\n\tpointerSmooth[ 0 ] = ${formatVector( this.parameters[i].pointerSmooth[0], digits )} mm \n\n\tpointerBuffer[end] = ${formatVector( this.parameters[i].pointerSmooth[Gestures.BUFFER_LENGTH - 1], digits )} mm`);
    ...
    updatePointerSmooth( i ) {
        ...
        _pointer.divideScalar( Gestures.WINDOW_SMOOTH ); // mm
    ...
    printDualParameters( digits ) {
        ...
        console.log(`parametersDual: \n\n\tvectorBuffer[ 0 ] = ${formatVector( this.parametersDual.vectorBuffer[0], digits )} mm \n\n\tpointerBuffer[end] = ${formatVector( this.parametersDual.vectorBuffer[Gestures.BUFFER_LENGTH - 1], digits )} mm`);
...
Gestures.CONTROLLER_DELAY = 20; // ms    

Gestures.DETECT_DELAY = 100; // ms

Gestures.BUFFER_LENGTH = 2; 

Gestures.WINDOW_SMOOTH = 1;

Gestures.TAP_DURATION_MAX = 150; // ms
Gestures.TAP_DISTANCE_MAX = 10; // mm

Gestures.POLYTAP_DURATION = 230; // ms

Gestures.SWIPE_DISTANCE_MIN = 3; // mm
Gestures.SWIPE_PATH_SPEED_MIN = 0.08; // m/s

Gestures.HOLD_START_DURATION_MIN = 500; // ms
Gestures.HOLD_START_DISTANCE_MAX = 3; // mm

Gestures.PAN_START_DISTANCE_MIN = 3; // mm
Gestures.PAN_PATH_SPEED_MAX = 0.08; // m/s

Gestures.PINCH_START_DIAMETER_OFFSET_MIN = 5; // mm
Gestures.PINCH_START_ANGLE_OFFSET_MAX = 10; // °
Gestures.PINCH_START_RADIAL_SPEED_MAX = 0.12; // m/s

Gestures.TWIST_START_DIAMETER_OFFSET_MAX = 5; // mm
Gestures.TWIST_START_DISTANCE_MIN = 30; // mm
Gestures.TWIST_START_ANGLE_OFFSET_MIN = 10; // °

Gestures.EXPLODE_DIAMETER_OFFSET_MIN = 11; // mm 
Gestures.EXPLODE_RADIAL_SPEED_MIN = 0.12; // mm 

Gestures.IMPLODE_DIAMETER_OFFSET_MAX = -11; // mm 
Gestures.IMPLODE_RADIAL_SPEED_MIN = 0.12; // mm 
