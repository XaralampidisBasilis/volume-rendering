...
sol = solve([eq1, eq2, eq4, eq5], [a, b, c, d], 'ReturnConditions', true);
...
%%
pol_t = 3*s*t - 3*t - 2*s - s*t^2 + t^2 + 3;
[params_t, terms_t] = coeffs(pol_t, t);
s_t = q * t * dot(params_t, terms_t);
pretty(s_t)

pol1_t = 6*s*t - 6*t - 2*s - 3*s*t^2 + 3*t^2 + 3;
[params1_t, terms1_t] = coeffs(pol1_t, t);
s1_t = (q/p) * dot(params1_t, terms1_t);
pretty(s1_t)
