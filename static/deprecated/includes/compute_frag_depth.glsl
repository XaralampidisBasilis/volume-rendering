 ...
 * @param frag_position: vec3 where the position of fragment is stored. The fragment is model coordinates
 * @param v_projection_model_view_matrix: a varying from the vertex shader that computes the mapping from model to clip space
 */
float compute_frag_depth(in vec3 frag_position)
{
    // Transform the fragment position to clip space
    vec4 clip_position = v_projection_model_view_matrix * vec4(frag_position, 1.0); 
